...
This report analyzes Airbnb listing price data using the GUIDE regression tree algorithm to identify the most influential predictors of listing price. The dataset, obtained from Kaggle, was preprocessed in Python to handle missing values, convert categorical variables, and apply a log transformation to the response variable. GUIDE was then used to construct a regression tree, which identified accommodates, beds, and bathrooms as the strongest predictors, with review_scores_rating serving as the root split variable.

To corroborate these findings, three additional models were trained in Python using scikit-learn: a linear regression baseline, a random forest trained on complete cases after dropping missing values, and a random forest trained on the full dataset after mean imputation. The random forest models achieved R2 scores of approximately 0.64–0.65 on a held-out test set, substantially outperforming linear regression at 0.45. Note that GUIDE's R2 of 0.72 reflects training data performance and is not directly comparable to these test-set scores. Across all models, accommodates consistently emerged as the dominant predictor of listing price, with property characteristics and review-related variables playing supporting roles.

To further corroborate the tree structure, a conditional inference tree was fit in R using ctree, which like GUIDE uses permutation-based statistical tests for variable selection. ctree identified accommodates and review_scores_rating as the most frequently used split variables, consistent with GUIDE's findings, and all splits were statistically significant under a Bonferroni-corrected threshold.
...
**Research question:** What are the most influential predictors of Airbnb listing prices, and do the GUIDE regression tree algorithm and Python-based models agree on their relative importance?

# Data Visualization

To better understand how listing characteristics relate to price, several exploratory plots are presented below. This data has been cleaned up, explanation for the cleanup is below this section.
...
# Corroborating GUIDE tree with R (ctree)
...
p <- ggplot(imp_df, aes(x = Normalized, y = reorder(Variable, Normalized), fill = Color)) +
  ...
  scale_fill_identity() +
  scale_x_continuous(expand = expansion(add = c(0, 3)), limits = c(0, 100), breaks = seq(0, 100, 20)) +
...
The difference in root split between GUIDE (review_scores_rating) and ctree (accommodates) does not indicate disagreement between the models. The root split identifies the single best first partition of the data, which is sensitive to the specific dataset and splitting criterion used. GUIDE selects the root split based on which variable best reduces prediction error in one step, while ctree selects based on the strongest statistical association. These are different criteria and can legitimately produce different root splits while still agreeing on the overall importance structure. The fact that both variables appear prominently in both trees (accommodates as GUIDE's top importance score and ctree's root, review_scores_rating as GUIDE's root and tied for top in ctree) suggests both are genuinely influential, with the ordering depending on how "importance" is measured rather than reflecting a true disagreement about which variables matter.

### Statistical significance

All splits are statistically significant, with every node in the ctree shows a p-value, and all are significant. This is meaningful because unlike GUIDE where most splits had gray circles (not statistically significant), every split ctree made passed the Bonferroni-corrected threshold. The variables that appear here are statistically supported, not just predictively useful.

### Key variables that appear 

accommodates, review_scores_rating, reviews_per_month, minimum_nights, beds all appear as split variables, consistent with what GUIDE and the random forests found.
...
# Corroborating GUIDE results with Python
...
### Random Forest (Imputing NaNs)

Similarly, another random forest with 500 trees was trained with the difference of training on the full dataset after mean imputation of missing values. Here is the code for the imputation:
...
Both a train/test split evaluation and a 10-fold cross-validation were applied to confirm consistency across evaluation methods, reusing the same code from before, but with the added imputed row values within the df:
...
**Explanation of trustworthiness of results:**
...
### Random Forest Importance (Dropped)
...
### Random Forest Importance (Imputed)
...
# Comparison of Importance Results

### Comparison between GUIDE and ctree

The only variable in the top 3 of both GUIDE and ctree is accommodates, which ranks first in both. Beyond that the rankings diverge, as GUIDE emphasizes property size variables (beds, bathrooms) while ctree emphasizes review-related variables (review_scores_rating, reviews_per_month). This difference is partly methodological: GUIDE's importance score weights splits by how much they reduced prediction error, favoring variables that make fewer but more impactful splits, while ctree's importance is a raw count of split appearances, favoring variables that appear repeatedly.

### Comparison between dropped and imputed

The top two variables are identical in both, in which accommodates is by far the most important in both cases, and bathrooms holds second place. The main differences are:

* `accommodates` dropped from 0.421 to 0.371, so it's slightly less dominant with imputation
* `bathrooms`' normalized score went up from 36.42 to 41.26 and is now relatively more important when accommodates weakens
* `reviews_per_month` got weaker (0.103 to 0.078)
* The middle variables shuffled slightly: 
  * `review_scores_rating` dropped from 4th to 6th
  * `number_of_reviews` moved up from 5th to 3rd
  * `host_acceptance_rate` moved up from 6th to 4th
  * `maximum_nights moved` up from 7th to 5th (tied area)
* `has_availability` went from exactly 0.000 to 0.001, essentially still zero but imputation recovered a tiny signal

This essentially means that the ranking shifts are minor and confined to the middle of the importance list. The top three variables remain unchanged in both position and relative dominance, and everything from minimum_nights downward holds its order exactly. The small reshuffling in the middle is expected given how similarly scored those variables are, where minor variation in the training data can cause reordering among near-tied variables. Overall, the two models agree strongly on which variables matter and by how much, suggesting that the choice of how to handle missing values has negligible impact on the conclusions drawn from variable importance.

### Comparison between GUIDE and Random Forest 

Both sets of models identify **accommodates** as the most important predictor overall based on variable importance scores.

Property characteristics such as bathrooms and beds rank highly in the GUIDE importance scores and are similarly influential in both random forest models, with the caveat of beds being in the lower end compared to GUIDE. These results indicate that structural features of the listing play an important role in determining price.

Review-related variables such as review_scores_rating, reviews_per_month, and number_of_reviews also appear among the important predictors across all three models. These variables likely capture aspects of guest satisfaction and listing popularity, which can influence demand and therefore pricing.

Although the exact importance rankings differ slightly between GUIDE and the random forest models, the two random forest variants themselves are in strong agreement (the minor reshuffling observed in the middle of the importance list has no bearing on the overall conclusions). The broad overlap in key predictors across all three models suggests that these relationships are robust across modeling approaches and missing data strategies, rather than artifacts of any specific algorithm or preprocessing choice.

--- 

# Conclusion
This analysis used the GUIDE regression tree algorithm to identify predictors of Airbnb listing prices and compared the results with models trained in Python and a conditional inference tree fit in R. Across all models, accommodates consistently emerged as the most influential predictor, suggesting that listing capacity is the primary driver of price. GUIDE additionally identifies beds and bathrooms as strong contributors, while the random forests emphasize bathrooms and reviews_per_month, and ctree highlights review_scores_rating and reviews_per_month alongside accommodates. The partial agreement across methods suggests that property characteristics and review-related variables have a consistent relationship with listing price, while differences in rankings reflect each method's sensitivity to different aspects of the data.

In the real world, guests also take missing information into account when booking. A listing without reviews or an unresponsive host may signal uncertainty, making guests less likely to book regardless of price. This suggests that missing values in variables like review_scores_rating and host_acceptance_rate are not purely a data quality issue, but may themselves reflect meaningful listing characteristics.

The most notable discrepancy across models is the role of beds, which ranks second in GUIDE but near the bottom in both random forests and ctree. This likely reflects the high correlation between beds, bathrooms, and accommodates. GUIDE may be capturing beds as a proxy for overall listing size, while the other models distribute that predictive credit more heavily onto accommodates and bathrooms.

Overall, the convergence across four distinct modeling approaches (GUIDE, ctree, and two random forest variants) on listing capacity and property characteristics as the primary drivers of Airbnb pricing lends strong confidence to these findings. While no two methods rank all variables identically, their agreement on the most dominant predictor is consistent and robust.
